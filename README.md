...
| POST | `/api/problems/:slug/run` | user | Sanity-check run against **sample test cases only** — same execution path as submit, but nothing is persisted, no score is computed. Shares the same rate-limit bucket as submit |
